...
def probarModelo(M,K,SNR_dB,precision,num_samples,L,modelo):
    model=modelo
    angles_de = np.arange(-90, 90, precision)
...
def entrenamiento(M,K,SNR_dB,precision,num_samples,lr,L):
  cont=0;
  preci = []
...
#   array_po = np.linspace(0, (M-1)/4, M)
  ...
  for epoch in range(num_epochs):
      ...
      media = sum(er) / len(er)
      mediaAbs= sum(e_absoluto) / len(e_absoluto)
...
erroresAbs=[]
for Kv in noises:
    red = entrenamiento(M=M, K=K, SNR_dB=Kv, precision=precision, num_samples=num_samples,lr=lr,L=signal)
