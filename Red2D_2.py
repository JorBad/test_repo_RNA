...
def datos2(angles_deg,array_pos,SNR_dB,K,num_samples,M,mapa,L,t,mapaphi,anglesphi):
  ...
  for m in range(num_samples):
    ...
    for n in range(K):#K
      # tmp=np.zeros((M*M,1),complex) ## ORIGNAL
...
def probarModelo(M,K,SNR_dB,precision,num_samples,L,modelo,t):
    ...
    for ss in ruidos:
        ...
        X_datax, y_labels =datos2(result2,array_po,ss,K,num_samples,M,mapas,L,t,mapas_phi,result_p)  
...
#   array_po = np.linspace(0, (M-1)/4, M)
  ...
  X_data, y_labels = datos2(result,array_po,SNR_dB,K,num_samples,M,mapas,L,t,mapas_phi,result_p)
...
def entrenamiento_datosiguales(M,K,SNR_dB,precision,num_samples,lr,L,t,epocas,X_data,y_labels):
  cont=0;
  preci = []
...
X_, y_ = datos2(result,array_po,SNR_dB,K,num_samples,M,mapas,L,t,mapas_phi,result_p)
for Kv in epo:
    ...
    # loss_t[:,nomb]= red[1]
    # loss_v[:,nomb]= red[9]
    red = entrenamiento_datosiguales(M=M, K=K, SNR_dB=SNR_dB, precision=precision, num_samples=num_samples,lr=lr,L=signal,t=t,epocas=Kv,X_data=X_,y_labels=y_)
